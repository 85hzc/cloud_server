...
{      
    ...
    if ("dev_get" == msgType) {
        ...
        var selectStr =  "SELECT "
            + "firmware_table.\"newestVersion\",iot_device.\"deviceId\",iot_dev_datamodel.\"firmwareId\" "
        ...
        client.query(selectStr, function(err, result) {
            ...
            var firmware_query = {
                ...
                newestVersion: result.rows[0].newestVersion,
        ...
        var selectStr =  "SELECT \"pluginId\",\"pluginName\",\"pluginDesc\",\"newestVersion\" "
            + "FROM plugin_table;"
        ...
        client.query(selectStr, function(err, result) {
            ...
            for(var i=0; i<result.rowCount; i++) {
                var pluginItem = {
                    ...
                    newestVersion: result.rows[i].newestVersion
                };
                pluginInstalled.data.push(pluginItem);
            }
        ...
        client.query(selectStr, function(err, result) {
            ...
            for (var i = 0; i < result.rowCount; i++) {
                var pluginDir = result.rows[i].pluginDir;
                var version = result.rows[i].newestVersion;
                var pluginId = result.rows[i].pluginId;
