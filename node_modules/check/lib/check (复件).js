...
function check_login(type, username, password, req, res)
{
    if ( "vendor_login" == type)
    {
        ...
        var select_query = client.query(select_string,function(err,result){
        	 if (err) {
            ...
            if(result[0]==undefined){
            	...
            	if(result[0].password != pwd){
            		...
            		console.log("qqq");
            		req.session.hasLogined = 1;
                	req.session.vendorID = result[0].vendorId;
                	req.session.vendorName = result[0].username;
                	res.redirect('/');
            	}
            }
...
function db_response_to_common_login_end_handler(result)
{
  ...
  /* if (result.length==0)
    ...
    else
    {
        ...
        if (this.rows[0].password != password)
        {
...
/*                this.res.redirect('/');
        ...
        else
        {
                ...
                this.req.session.vendorID = this.rows[0].vendorId;
                this.req.session.vendorName = this.rows[0].username;
