...
function check_input(type, req, res)
{
    ...
    if ( "vendor_id" == type)
    {
        ...
        var select_string = "SELECT * FROM " + db.vendor_table + " WHERE vendor_ID = '" + vendorID + "'";
...
function db_response_to_common_register_handler(select_result)
{
    ...
    if ( 0 == select_result.rowCount)
    {
        var insert_string = "INSERT INTO " + db.vendor_table + " VALUES ('"
                          + this.username +"','"
                          + this.password +"','"
                          + this.vendorID + "')";
...
function check_register(type, username, password, vendorID, req, res)
{
    if ( "vendor_register" == type)
    {
        ...
        select_query.vendorID = vendorID;
...
//    console.log(result);
	...
	if ( this.rows[0].password != this.password)
	{
	...
	else
	{
            ...
            this.req.session.vendorID = this.rows[0].vendor_id;
