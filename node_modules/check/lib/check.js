...
function check_input(type, req, res)
{
    ...
    if ( "username" == type)
    {
        ...
        var select_string = 'SELECT * FROM vendor  WHERE username ="'+username+'"';
        console.log(select_string);
        var select_query = client.query( select_string);
        select_query.res = res;
        select_query.on('result', database_response_to_select_handler);
    ...
    if ( "vendor_id" == type)
    {
        ...
        var select_string = "SELECT * FROM vendor  WHERE vendorid = '" + vendorID + "'";
...
function database_response_to_select_handler(row)
{
...
function db_response_to_common_register_handler(row)
{
    ...
    if ( 0 == row.lenth)
    {
        ...
        var insert_string = "INSERT INTO vendor (username,password,email) VALUES ('"
                          + this.username +"','"
                          + password +"','"
                          + this.email + "')";
...
function check_register(type, username, password, email, req, res)
{
    if ( "vendor_register" == type)
    {
        ...
        select_query.res = res;
        select_query.on('result', db_response_to_common_register_handler);            
    }
}*/
...
function check_register(type, username, password, email, req, res)
{
    if ( "vendor_register" == type)
    {
        ...
        {
            req.session.hasLogined = 0;
        }

        insert_query.req = req;
        insert_query.res = res;
        insert_query.on( 'end', function(result)
{
    res.render('login');
});
      }
...
function row_push(result)
{
    this.rows.push(result);
}

function db_response_to_common_login_end_handler(row)
{

    console.log(row);
    console.log(row.vendorId);
    console.log("db_response_to_common_login_end_handler");
    ...
    else
    {
        ...
        {
                this.req.session.hasLogined = 1;
                this.req.session.vendorID = row.vendorId;
                this.req.session.vendorName = row.username;
                //console.log(this.req.session.vendorID);
                //console.log(this.req.session.hasLogined);
                //this.res.render('welcome')
                this.res.redirect('/');
        }
...
function check_login(type, username, password, req, res)
{
    if ( "vendor_login" == type)
    {
        var select_string = 'SELECT * FROM vendor WHERE username ="'+username+'"';
        console.log(select_string);
        ...
        select_query.req = req;
        select_query.res = res;
        select_query.rows = new Array();
        //select_query.on('row', row_push);       
        select_query.on('result', db_response_to_common_login_end_handler); 
        select_query.on('end',function(){
            this.res.render('login');
...
module.exports.file_delete = file_delete;
