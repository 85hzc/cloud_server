...
module.exports.server_action_handler = server_action_handler;
...
function server_action_handler(action, sn, ipk_name)
{
    var i = 0;
...
function handle_client_offline(socket)
{
    ...
    this.register_obj.socket.online_status = false;
...
function response_to_client_register( register_obj, status)
{
    ...
    var end_str = "{\"type\":\"resp_register\",\"status\":\"" + status + "\"}";
...
function database_response_to_update_handler(update_result)
{
    ...
    if ( true == this.register_obj.socket.online_status)
    {
        console.log("online true 2");
        response_to_client_register(this.register_obj, 2);
        socket_array_handler(socket_array_obj, true);
    }

    /* false , means the client is offline */
    else
    {
        console.log("online false 2");
        socket_array_handler(socket_array_obj, false);
    }
...
{
        var update_string = "UPDATE cloud_ac SET online = '"
...
function database_response_to_select_handler(select_result)
{
    ...
    if ( 0 == select_result.rowCount)
    {
        ...
        var insert_string = "INSERT INTO cloud_ac VALUES ('"
                          + this.register_obj.http_req.body.sn + "','" 
                          + this.register_obj.http_req.body.mac + "','" 
                          + true + "')";
        var insert_query = client.query(insert_string);
        insert_query.register_obj = this.register_obj;
    ...
    else
    {
        ...
        var update_string = "UPDATE cloud_ac SET online = 'true' WHERE sn = '" 
                          + this.register_obj.http_req.body.sn + "'";
        var update_query = client.query( update_string);
        update_query.register_obj = this.register_obj;
        update_query.on('end', database_response_to_update_handler); 
    }
}
...
function client_packet_handler( req, res)
{
   ...
   // console.log(req);
    if ( req.body.sn)
    {
        ...
        register_obj.socket.online_status = true;
