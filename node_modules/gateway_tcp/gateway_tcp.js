...
// client.connect();
//var g_zbsrvurl = "http://172.17.225.249:3001/zb";
var g_zbsrvurl = "http://172.17.225.249:8090/zb/";
...
function send_result(sock, retCode, errStr, commandId) {
    ...
    send_gw_result(sock, JSON.stringify(ret));
}
...
function select_row_handler(row)
{
   this.rows_arr.push(row);
}
...
function insert_end_handler(row)
{
    var endResult = {
        ret: 0,
...
function insert_device_handler(row)
{
    if (0==row.length) {
		...
		//query.on('result', select_row_handler);
        query.on('result', insert_end_handler);
    }
}

function device_online_handler(row)
{
    ...
    if ( 0==row.length) {
        ...
        //query.on('result', select_row_handler);        
        query.on('result', insert_device_handler);
    } else {
        /* already in iot_device table, update it */
        /* TODO: is gatewayId needed here? */
        var update_string = "UPDATE iot_device"
                           + " SET online = '1', gatewayId = '" + "1"
                           +   "' , devData = '"    + JSON.stringify(this.setObj.devData)
                           + "' WHERE manufacture ='" + row.manufacture
                           + "' AND manufactureSN = '" + row.manufactureSN + "';";
        ...
        /* update device's socket mapping table */
        devConn.updateDevInfo(row.deviceId, this.setObj.skId);
...
{ 
    var head = message.slice(0, tcpHeadLen);
    var msgLen = (head[1] & 0xff)  | ((head[0] & 0xff) << 8);
    var msgStr = message.toString('ascii', 0, message.length);
    var jsonStr = msgStr.substr(tcpHeadLen, msgLen);

    console.log("json length is ", msgLen + ", message length is " +  message.length);
    ...
    else if ("device_online" == msg.type)
    {
        ...
        var beMaster= devData.beMaster; 
        var ipAddr = devData.ipAddr;
       ...
       client.query(select_str,function(err,result){
            if(result.length < 1){

                var insertStr="INSERT INTO iot_device (manufacture, manufactureSN,beMaster,online,ipAddr) VALUES('"+manufacture+"','"+manufactureSN+"','"+beMaster+"','1','"+ipAddr+"') ";
                ...
                deviceId:result3[0].ID.toString(),
                zburl:g_zbsrvurl
                };

                console.log(endResult);
                send_gw_result(sock, JSON.stringify(endResult)+"\r\n");
                //console.log("sock:"+sock);
                console.log("sock:"+sock.id);

                devConn.updateDevInfo(result3[0].ID, sock.id);
                ...
                var updateStr="UPDATE iot_device set online='1' , beMaster='"+beMaster+"',ipAddr='"
                    + ipAddr + "' WHERE deviceId='"
                    + result[0].deviceId + "';";

                    console.log(updateStr);
                    client.query(updateStr,function(err,result3){
                ...
                manufacture: manufacture,
                manufactureSN: manufactureSN,
                deviceId:result[0].deviceId.toString(),
                zburl:g_zbsrvurl
                };
               ...
               // console.log("sock:"+sock);
                console.log("sockId:"+sock.id);
               // console.log(result[0].deviceId);
                devConn.updateDevInfo(result[0].deviceId, sock.id);
    ...
    else if ("device_offline" == msg.type)
    {
        var devData=msg.devData;
        ...
        if(typeof(manufacture) == 'undefined' ||
           typeof(manufactureSN) == 'undefined' ) {
            send_result(sock, 70003, "missing manufacture or manufactureSN", cmdId);
            return;
        }
        ...
        client.query(select_str, function(err, result) {
            if (err) {
                ...
                send_result(sock, 9001, "server error", cmdId);
                return;
            }

            if (result.length != 1) {
                console.log("query iot device error");
                send_result(sock, 9001, "server error", cmdId);
                return;
            }

            var updateStr = "UPDATE iot_device set online='0' , devData='"
                    + JSON.stringify(devData) + "' WHERE deviceId='"
                    + result[0].deviceId + "';";
    ...
    else if ( "rsp_set" == msg.type)
    {
        ...
        client.query(selectStr, function(err, result) {
            ...
            var newDevData =  JSON.parse(result[0].devData);
            console.log(newDevData);
            ...
            console.log(JSON.stringify(newDevData));
    ...
    else if ("lirc_checklib" == msg.type) {
        console.log("Receive check lirc code message from adapter");

        var commandId = msg.commandId;
        var deviceId = msg.deviceId;

        var selectStr = "SELECT max(id) as id FROM lirc_code;";
        client.query(selectStr, function(err, result) {
            if (err) {
                console.error(err.stack);
                return;
            ...
            var queryStr = "SELECT version,downloadUrl FROM lirc_code WHERE id='"
                    + result[0].id + "';";

            console.log(queryStr);

            client.query(queryStr, function(err, result1) {
                if (err) {
                    console.error(err.stack);
                    return;
                }
                ...
                var retStr = {
                    type: 'rsp_lirc_checklib',
                    commandId: commandId,
                    deviceId: deviceId,
                    version: result1[0].version,
                    url: result1[0].downloadUrl
                };

                send_gw_result(sock, JSON.stringify(retStr)+"\r\n");
            });
        });
    }
}
