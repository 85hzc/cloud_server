...
function message_handle(req, res){      
    ...
    if ("dev_get" == msgType) {
        ...
        if (typeof(userId) == "undefined" || typeof(deviceType) == "undefined") {
            send_result(res, msgType, 80001, "参数错误！");
            return;
        }
        ...
        client.query(selectStr, function(err, result) {
            if (err) {
                ...
                send_result(res, msgType, 90001, "数据库查询错误！");
                return;
            }
        ...
        client.query(selectStr, function(err, result) {
            ...
            if(result.rowCount != 1) {
                send_result(res, msgType, 90002, "后台数据库错误！");
                return;
            }

            console.log(JSON.stringify(result.rows[0]));
            send_result(res, msgType, 0, result.rows[0]);
        });
    }
        ...
        client.query(selectStr, function(err, result) {
            ...
            if(result.rowCount < 1) {
                send_result(res, msgType, 90009, "设备不存在！");
                return;
            }
            ...
            if(result.rows[0].online == false) {
                send_result(res, msgType, 90003, "设备离线！");
                return;
            }
            ...
            if (-1 == socket) {
                send_result(res, msgType, 90004, "后台与设备连接错误！");
                return;
            }
            ...
            send_result(res, msgType, 0, "Success!");
        });
    }
...
/*修改*/
   ...
   /* else if("lirc_set"== msgType){
    ...
    else if ("monitor_dev_req" == msgType) {
        //var devId = message.devId;

        var selectStr =  "SELECT \"deviceid\" FROM monitor_device;";
        ...
        client.query(selectStr, function(err, result) {
            ...
            for(var i=0; i<result.rowCount; i++) {
                ...
                dataAvailable.devList.push(dataItem);
                */
				dataAvailable.devList[i] = result.rows[i].deviceid;
            }
            console.log(JSON.stringify(dataAvailable));
            res.send(JSON.stringify(dataAvailable));
        });
    }
    else if ("upstream_flow_sts" == msgType) {
        var devId = message.devId;

        var selectStr =  "SELECT \"sip\",\"dip\",\"bw\" "
            + "FROM upstream_flow_raw;";
        ...
        client.query(selectStr, function(err, result) {
            ...
            for(var i=0; i<result.rowCount; i++) {
                var dataItem = {
                    sip: result.rows[i].sip,
                    dip: result.rows[i].dip,
                    bw: result.rows[i].bw
                };
                dataAvailable.monitorList.push(dataItem);
            }
            console.log(JSON.stringify(dataAvailable));
            res.send(JSON.stringify(dataAvailable));
        });
